...
motion_core::Result<void> MksAxisAdapter::start() {
    ...
    if (std::abs(config_.axis_units_per_degree) < 1e-9) {
        started_ = false;
    ...
    if (pos_res.ok()) {
        ...
        if (payload.size() >= 6) {
            target_position_deg_.store(static_cast<double>(MksProtocol::readBe48s(payload.data())) / config_.axis_units_per_degree, std::memory_order_relaxed);
        } else if (payload.size() == 4) {
            target_position_deg_.store(static_cast<double>(static_cast<std::int32_t>(MksProtocol::readBe32s(payload.data()))) / config_.axis_units_per_degree, std::memory_order_relaxed);
        }
    }
...
motion_core::Result<void> MksAxisAdapter::apply_command(const motion_core::AxisCommand& command) {
    ...
    if (command.has_target_position) {
        ...
        if (command.is_relative) {
            target_position_deg_.store(current_target + command.target_position_deg, std::memory_order_relaxed);
        } else {
...
void MksAxisAdapter::process_cycle(MksProtocol& protocol) {
    ...
    if (set_zero_req_.exchange(false, std::memory_order_relaxed)) {
        std::vector<uint8_t> r;
    ...
    if (has_new_target_pos_.exchange(false, std::memory_order_acquire)) {
        ...
        const double raw_axis = signed_target_deg * config_.axis_units_per_degree;
    ...
    if (has_new_target_vel_.exchange(false, std::memory_order_acquire)) {
        ...
        case 2: // Encoder position
            if (protocol.sendCommand(runtime_can_id, MksCommand::ReadEncoderAddition, {}, payload,
                ...
                if (payload.size() >= 6) {
                    ...
                    const double position_deg = static_cast<double>(t_update.raw_axis_position) / config_.axis_units_per_degree;
        ...
        switch (parameter) {
            ...
            case MksParameter::CanId:
                if (auto r = mks::require_unsigned_value(entry.value, "uint"); r.ok()) {
                    ...
                    const auto status_result = ensure_write_status_success(config_.bus_manager->execute_raw_command_sync(
                        old_can_id, static_cast<uint8_t>(MksCommand::SetCanId), payload));
                    if (!status_result.ok()) return status_result;
