...
motion_core::Result<void> MksCanBusManager::register_adapter(const std::uint16_t can_id, MksAxisAdapter* adapter) {
    ...
    if (adapter_registry_.find(can_id) != adapter_registry_.end()) return motion_core::Result<void>::failure({motion_core::ErrorCode::AlreadyExists, "exists"});
...
void MksCanBusManager::update_stats() {
...
void MksCanBusManager::poll_cycle() {
    ...
    // Process adapters
    std::vector<MksAxisAdapter*> adapters_to_process;
    {
        std::lock_guard<std::mutex> lock(state_mutex_);
        if (!is_started_locked() || axis_order_.empty()) return;
        
        for (auto id : axis_order_) {
            auto it = adapter_registry_.find(id);
            if (it != adapter_registry_.end() && it->second) {
                adapters_to_process.push_back(it->second);
            }
        }
    }

    for (auto* adapter : adapters_to_process) {
        adapter->process_cycle(*protocol_);
    }
}
