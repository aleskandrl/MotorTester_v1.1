    ...
    if (sine_enabled_.load(std::memory_order_acquire)) {
        if (!sine_stopped_due_to_drops_ && dropped > last_driver_dropped_ && manager_) {
            ...
            const QString message = QStringLiteral(
                                        "EtherCAT Axis %1: motion queue drops detected (%2 new). "
                                        "Stopping sine producer to avoid UI/event flood. "
                                        "Check mode switch confirmation and drive readiness.")
                                        .arg(axis_id_)
                                        .arg(new_dropped);
...
void EthercatAxisWorkspace::onTransportTelemetryUpdated(const QVariantMap& telemetry, const QString& transport, double t_sec) {
    ...
    if (show_position
        ...
        && telemetry.contains(QStringLiteral("target_position_deg"))
        && !sine_enabled_.load(std::memory_order_acquire)) {
        h.scope->addData(QStringLiteral("target"),
                         t_sec,
                         telemetry.value(QStringLiteral("target_position_deg")).toDouble());
    }
    ...
    if (telemetry.contains(QStringLiteral("mode")) && h.mode_combo && !h.mode_combo->hasFocus()) {
        ...
        const auto mode_u64 = static_cast<std::uint64_t>(mode);
        if (last_driver_mode_ != mode_u64) {
            last_driver_mode_ = mode_u64;
            if (sine_stopped_due_to_drops_ && supportsSineMode()) {
                sine_stopped_due_to_drops_ = false;
                drop_baseline_initialized_ = false;
                last_driver_dropped_ = 0U;
                if (manager_) {
                    auto* manager = manager_.data();
                    const QString message = QStringLiteral(
                        "EtherCAT Axis %1: mode changed to %2, sine producer guard re-armed.")
                        .arg(axis_id_)
                        .arg(mode);
                    QMetaObject::invokeMethod(
                        manager,
...
void EthercatAxisWorkspace::onBeforeHomeAxis() {
    if (control_panel_ && control_panel_->handles().mode_combo
        && control_panel_->handles().mode_combo->currentData().toInt()
            == static_cast<int>(motion_core::AxisMode::ManualHoming)) {
        pending_refill_points_ = 0U;
        motion_queue_prefilled_ = false;
